...
##################################### By Maysaa 25/06/2019 ############################
@frappe.whitelist()
def printt_via_printnode(**kwargs):
	...
	if not frappe.db.exists("Print Node Action", {'user':user,'action':action_type}):
		frappe.throw(
			_("Unable to find an action in Print settings to execute this print")
		)
	else:
		action = frappe.get_doc("Print Node Action", {'user':user,'action':action_type})
		if not kwargs.get('doctype') and action.get('print_format'):
			kwargs['doctype'] = frappe.db.get_value('Print Format', action.print_format, 'doc_type')
	...
	#if action_type=='Beverage': print_format_type = 'beverage_print_format' 
	#elif action_type=='Kitchen': print_format_type = 'kitchen_print_format' 
	if action_type=='POS' and kwargs.get("doctype") == 'Sales Invoice': print_format_type = 'Maharaja GST POS Invoice' 
	#elif action_type=='POS' and kwargs.get("doctype") =='POS': print_format_type = 'Maharaja Point of Sale'
	else: print_format_type = action.print_format 
